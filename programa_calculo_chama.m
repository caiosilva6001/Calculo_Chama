...
while k<=7
    ...
    disp(" A ordem dos aparatos deve ser: G3, G2, G1, M3, P3, LG-TEO, Q1 ");
    disp(['Para o aparato ' num2str(k) ' insira os parametros: ']);

    altura_aparato_mm(k)= input('Digite a altura (mm): ');
    ...
    while i<=5
      ...
      disp("\n Entradas \n\n");
      disp(['Altura_aparato: ' num2str(altura_aparato_mm(k)) ' ' 'mm']);
      ...
      razao_equivalencia(i)=21.993/(sqrt(delta_p_ar_pa(i)/delta_p_fuel_pa(i)));
      incerteza_razao_equivalencia(i)=sqrt(((30.2308*(delta_p_ar_pa(i)^2))+(30.2308*(delta_p_fuel_pa(i)^2)))/((delta_p_ar_pa(i)^3)*(delta_p_fuel_pa(i))));
      alfa_rad(i)=deg2rad(alfa_graus(i));
      velocidade_queima(i)=(((19.66*(10^(-6))*(delta_p_ar_pa(i)^(1/2)))+(28.084*(10^(-6))*(delta_p_fuel_pa(i)^(1/2))))*sin(alfa_rad(i)))/(1.2301*A_fenda_m2(k));
      ...
      incerteza_velocidade_queima_parcela_2=(3.25775*(10^-11)*((sin(alfa_rad(i)))^2))/(((A_fenda_m2(k))^2)*delta_p_fuel_pa(i));
      ...
      reynolds(i)=(0.1258653*a(k)*b(k)*((19.66*(delta_p_ar_pa(i)^(1/2)))+(28.084*(delta_p_fuel_pa(i)^(1/2))))*sin(alfa_rad(i)))/((a(k)+b(k))*A_fenda_m2(k));
      incerteza_reynolds_parcela_1(i)=(0.382701*(a(k)^2)*(b(k)^2)*((sin(alfa_rad(i)))^2))/(((A_fenda_m2(k))^2)*delta_p_ar_pa(i)*((a(k)+b(k))^2));
      incerteza_reynolds_parcela_2(i)=(0.780926*(a(k)^2)*(b(k)^2)*((sin(alfa_rad(i)))^2))/(((A_fenda_m2(k))^2)*delta_p_fuel_pa(i)*((a(k)+b(k))^2));
    ...
    p=input('Digite 1  para calcular os parâmetros do proximo aparato ,  ou  digite  2 para parar:  ');
    if p==2
        break;
    end
    disp('Digite os parametros do proximo aparato: ');
end

% Definicao de aparatos
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Comparação Razao de Equivalencia versus Su para diferentes Aparatos com Fenda 1x15 mm')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Comparação Razao de Equivalencia versus Su para diferentes Aparatos com Fenda 1x15 mm e Literatura')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Comparação Razao de Equivalencia versus Su com Aproximação Polinomial para diferentes Aparatos')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Comparação Reynolds versus Su para diferentes alturas de Aparato com Fenda 1x15mm')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Reynolds versus Su - Aparato G3 (108 mm de altura)')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Reynolds versus Su - Aparato M3 (95 mm de altura)')
...
ylabel('Velocidade de chama[cm/s]')
grid on
title('Reynolds versus Su - Aparato P3 (72 mm de altura)')
